                ...
                <div class="table-responsive">
                    <table class="table table-striped table-hover">
                        ...
                        <tr>
                            ...
                            <th style="width: 12%"><span class="glyphicon glyphicon-comment"></span> <span
                                    class="visible-lg">回复</span></th>
                            <th style="width: 13%"><span class="glyphicon glyphicon-time"></span> <span
...
<!--查看评论模态框-->
<div id="seeMessage" tabindex="1" role="dialog" style="display: none">
    <table class="table" style="margin-bottom:0px;">
        <tbody>
        <tr>
            <td wdith="20%">留言ID:</td>
            <td width="80%" class="see-message-id" id="message_id"></td>
        </tr>
        <tr>
            <td wdith="20%">留言用户:</td>
            <td width="80%" class="see-message-id" id="nickname" rel=""></td>
        </tr>
        <tr>
            <td wdith="20%">留言内容:</td>
            <td width="80%" class="see-message-content" id="message_content"></td>
        </tr>
        <tr>
            <td wdith="20%">IP:</td>
            <td width="80%" class="see-message-ip" id="message_ip"></td>
        </tr>
        <tr>
            <td wdith="20%">所在地:</td>
            <td width="80%" class="see-message-address" id="message_adds"></td>
        </tr>
        <tr>
            <td wdith="20%">系统:</td>
            <td width="80%" class="see-message-system" id="message_os"></td>
        </tr>
        <tr>
            <td wdith="20%">浏览器:</td>
            <td width="80%" class="see-message-browser" id="message_browser"></td>
        </tr>
        <tr>
            <td wdith="20%" style="border-bottom: 1px #ddd solid">留言时间:</td>
            <td width="80%" style="border-bottom: 1px #ddd solid" class="see-message-browser" id="message_time"></td>
        </tr>
        </tbody>
    </table>
</div>
...
<!--回复模态框-->
<div id="replyMessage" tabindex="1" role="dialog" style="display: none">
    <div style="width: 100%;padding: 20px 15px 0">
        <input type="hidden" id="reply_message_id" value="">
...
<script>
    $(function () {
        ...
        function load() {
            ...
            $.ajax({
                type: "post",
                url: "${path}/messageInfo/selectMessage",
                timeout: 10000,
                data: param,
                success: function (data) {
                    ...
                    $.each(data.data, function (index, item) {
                        var comment = item.message_content;
                        comment = comment.length > 50 ? comment.substring(0, 50) + "..." : comment;
                        $("#table-content").append("<tr>" +
                            "<td><input type='checkbox' class='input-control' name='checkbox[]' value='" + item.message_id + "'/></td>" +
                            "<td class='comment-content'>" + comment + "</td>" +
                            "<td><a class='seeReply' rel='" + item.message_id + "' data-reply='0' title='查看回复'>" + item.reply_num + "</a></td>" +
                            "<td>" + item.message_time + "</td>" +
                            "<td><a class='detailMessage' rel='" + item.message_id + "' title='查看留言'>查看</a> <a class='deleteMessage' rel=" + item.message_id + ">删除</a></td></tr>");
                    });
        ...
        $("body").on('click', '.detailMessage', function () {
            var event = $(this);
            if (event.attr("data-message") == 1)
                return false;
            event.attr("data-message", '1');
            var message_id = event.attr("rel");
            var title = event.parent().parent().find('.comment-content').text();
            title = title.length > 10 ? title.substring(0, 10) + "..." : title;
            $.ajax({
                type: "post",
                url: "${path}/messageInfo/seeMessage",
                timeout: 10000,
                data: {message_id: message_id},
                success: function (data) {
                    $.each(data[0], function (index, item) {
                        if (index == "message_id")
                            $("#message_id").attr("rel", item);
                        else if (index == "user_id")
                            $("#nickname").attr("rel", item);
                        $("#" + index + "").text(item);
                    });
                    seeMessage(event, title);
                }, complete: function (XMLHttpRequest, status) {
                    if (status == "timeout") {
                        layer.alert('网络异常，请检查网络', {
                            icon: 2,
        ...
        $("body").on('click', '.deleteReply', function () {
            ...
            layer.confirm('确认删除？', {
                ...
                ajaxDelete(id, '${path}/m_replyInfo/deleteReply');
            }, function () {
                layer.close(layer.index);
            });
        });
        ...
        $("body").on('click', '.deleteMessage', function () {
            ...
            layer.confirm('确认删除？', {
                ...
                ajaxDelete(id, '${path}/messageInfo/deleteMessage');
            }, function () {
                layer.close(layer.index);
            });
        });
        ...
        $("body").on('click', '.batchDeleting', function () {
            ...
            layer.confirm('确认删除？', {
                ...
                ajaxDelete(ids, '${path}/messageInfo/deleteMessage');
            }, function () {
                layer.close(layer.index);
            });
        });
         ...
         *
         * */
        function seeMessage(event, title) {
            var seeMessage = $("#seeMessage").html();
            var p_index = layer.open({
                type: 1
                , content: seeMessage
                , title: '查看留言(' + title + ')'
         ...
         *
         * */
        function replyMessage(data,p_index,b_index) {
            $("#reply_message_id").val(data.get("message_id"));
            $("#reply_user_id").val(data.get("user_id"));
            $("#reply_content_info").attr("placeholder", "回复 " + data.get("user_name") + ":");
            var replyMessage = $("#replyMessage").html();
            var o_index = layer.open({
                ...
                , yes: function (index, layero) {
                    ...
                    var param = {};
                    param.message_id = layero.find("#reply_message_id").val();
         ...
         *
         * */
        function addReply(param, o_index,p_index,b_index) {
            $.ajax({
                type: "post",
